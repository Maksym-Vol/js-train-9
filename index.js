...
function findElementGreaterThan(arr, num) {
  ...
  return arr.find(element => element > num) ?
    arr.find(element => element > num) : 'Числа яке задовільняе умову не знайдено';
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map(num => num ** 2);
  const filteredSquaredArray = squaredArray.filter(num => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastElements.reduceRight((acc, num) => acc + num, 0);
  lastElements.forEach(num => console.log(num));  return sum;
}
...
function expandArrayByN(arr, n) {
 ...
 if (!Array.isArray(arr) || typeof n !== 'number') {
  ...
  let newArr = arr.flatMap(num => Array(n).fill(num));
  return newArr;
}
